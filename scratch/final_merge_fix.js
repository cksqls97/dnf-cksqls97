...
// 1. Fix Table Header
content = content.replace(/<th style=\{\{ padding: '0\.2rem 0\.1rem', borderLeft: '1px solid rgba\(255,255,255,0\.1\)', fontSize: '0\.7rem' \}\}>닳아버린<br\/>순례의 증표 \(단가\)<\/th>\s*<th style=\{\{ padding: '0\.2rem 0\.1rem', fontSize: '0\.7rem' \}\}>레시피 \(구매\/순례의 인장\/판매\)<\/th>/s, `<th colSpan="2" style={{ padding: '0.2rem 0.1rem', borderLeft: '1px solid rgba(255,255,255,0.1)', fontSize: '0.7rem' }}>특별상점 (증표 구매 / 레시피 제작)</th>`);

// 2. Fix Table Row Buttons
content = content.replace(/\{\/\* 16 \*\/\} <td style=\{\{ padding: '0\.2rem 0\.1rem', borderLeft: '1px solid rgba\(255,255,255,0\.1\)', verticalAlign: 'middle', minWidth: '80px' \}\}>.*?<\/td>\s*\{\/\* 17 \*\/\} <td style=\{\{ padding: '0\.2rem 0\.1rem', verticalAlign: 'middle', minWidth: '80px' \}\}>.*?<\/td>/s, `{/* 16-17 특별상점 */} 
...
// 3. Fix SecretShopModalComponent Logic & Rendering
const modalStart = 'function SecretShopModalComponent({ activeSecretShopModal, setActiveSecretShopModal, characters, getCharForm, addCharToken, updateCharToken, removeCharToken, addCharRecipe, updateCharRecipe, removeCharRecipe, updateCharForm }) {';
const modalEnd = '  );\n}';

const fullModalReplacement = `function SecretShopModalComponent({ activeSecretShopModal, setActiveSecretShopModal, characters, getCharForm, addCharToken, updateCharToken, removeCharToken, addCharRecipe, updateCharRecipe, removeCharRecipe, updateCharForm }) {
  useEffect(() => {
...
const modalFullPattern = /function SecretShopModalComponent[\s\S]*?\}\s*\}\s*;/;
// Actually, let's just find from function start to the end of the return block.
const startIdx = content.indexOf(modalStart);
// Find the last return line
const lastLine = '  );' + '\n' + '}';
const endIdx = content.indexOf(lastLine, startIdx) + lastLine.length;

const originalModal = content.substring(startIdx, endIdx);
content = content.replace(originalModal, fullModalReplacement);

fs.writeFileSync(path, content, 'utf8');
console.log('Final Merge & Fix script finished.');
