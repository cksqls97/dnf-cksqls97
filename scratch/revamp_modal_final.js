...
// 1. Logic Update
const logicPattern = /useEffect\(\(\) => \{[\s\S]*?setActiveSecretShopModal\(null\);/s;
...
content = content.replace(logicPattern, newLogic);

// 2. Section Update
const sectionPattern = /\{activeSecretShopModal\.type === 'token' && \([\s\S]*?\}\)\s*\{activeSecretShopModal\.type === 'recipe' && \([\s\S]*?\}\)/;
const newSections = `
...
if (sectionPattern.test(content)) {
    content = content.replace(sectionPattern, newSections);
    fs.writeFileSync(path, content, 'utf8');
    console.log('Modal revamped successfully.');
} else {
    console.log('Modal section pattern not found.');
}
